...
class IGraphEngineManager // Взят из библиоткеи двумерных примитивов
{
public:
...
class GraphEngineManager : public IGraphEngineManager
{
protected:
    GraphEngineManager() {}
    static GraphEngineManager* graphEngineManager;
public:
    void operator =(const GraphEngineManager& other) = delete;
    GraphEngineManager(GraphEngineManager&) = delete;
    void Activate(const char* key) override {} 
    bool ActivateState() const override { return true; }
    int  LastErrorCode() override { return 0; }
    static GraphEngineManager* GetInstance()
    {
        if (graphEngineManager == nullptr)
        {
            graphEngineManager = new GraphEngineManager();
        }
        return graphEngineManager;
    }
    ~GraphEngineManager()
    {
        delete graphEngineManager;
    }
};

GraphEngineManager* GraphEngineManager::graphEngineManager = nullptr;
...
class Figure : public ITextEditorItem
{
  ...
  IGraphEngineManager* geometricEngine;
  
public:
  
  Figure( int col, int back, IGraphEngineManager& geomToolkit, const char* keyToActivate )
